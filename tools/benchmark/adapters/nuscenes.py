...
Expects the user to have pre-converted nuScenes to KITTI-like format:
  <data_root>/
    <seq_int>/
...
Alternatively, if using nuscenes-devkit directly, a future helper
`tools/benchmark/convert_nuscenes.py` can produce this layout.

This adapter sets `has_native_int_ids = False` so the pipeline writes
a sidecar `_index.json` mapping int IDs back to tokens.
"""
...
class NuScenesAdapter(BenchmarkAdapter):
    ...
    def __init__(
        ...
        self._native_frame_ids: dict[int, dict[int, str]] = {}
    ...
    def _load_seq(self, seq: int) -> None:
        ...
        # Native scene token
        token_file = os.path.join(seq_dir, "scene_token.txt")
        ...
        # Native sample tokens
        sample_file = os.path.join(seq_dir, "sample_tokens.txt")
        self._native_frame_ids[seq] = {}
        if os.path.exists(sample_file):
            with open(sample_file) as f:
                for i, line in enumerate(f):
                    self._native_frame_ids[seq][i] = line.strip()
        else:
            for i in range(n):
                self._native_frame_ids[seq][i] = str(i)
    ...
    def native_frame_id(self, seq: int, frame_id: int) -> str:
        return self._native_frame_ids.get(seq, {}).get(frame_id, str(frame_id))
