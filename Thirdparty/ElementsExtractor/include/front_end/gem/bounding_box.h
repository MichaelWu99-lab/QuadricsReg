...
namespace BBoxUtils {
    ...
    struct RotatedRect {
        ...
        RotatedRect(const Eigen::Matrix3Xd &matrix, const Eigen::Matrix3d &basis, double degrees) {
            Eigen::Matrix3d R_12 = Eigen::AngleAxisd(degrees * 3.14159 / 180.,
                                                     Eigen::Vector3d::UnitZ()).toRotationMatrix();

            basis_ = basis * R_12;
            Eigen::Matrix3Xd newMatrix =
                    basis_.transpose() * matrix; // transform the points from world frame to local frame
            ...
            for (int i = 0; i < newMatrix.cols(); i++) {
                ...
                double dist = std::min(std::min(distLeftBottom(0), distLeftBottom(1)),
                                       std::min(distRightTop(0), distRightTop(1)));
                if (distLeftBottom(0) < 0 || distLeftBottom(1) < 0 || distLeftBottom(0) > size_(0) ||
                    distLeftBottom(1) > size_(1)) {
                    score += dist * 1.414;
                } else {
                    score += dist;
                }
