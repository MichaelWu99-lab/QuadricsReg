...
//            score = lambda_[0] * lambda_[1] * lambda_[2];
    ...
    void QuadricFeature::fitting() {
        ...
        if (method == "PCA") {
            ...
            Eigen::Vector3d bottomLeft_w =
                    Rwb.transpose() * Eigen::Vector3d(cloud_->points[0].x, cloud_->points[0].y, cloud_->points[0].z);
            Eigen::Vector3d topRight_w =
                    Rwb.transpose() * Eigen::Vector3d(cloud_->points[0].x, cloud_->points[0].y, cloud_->points[0].z);
    ...
    Eigen::VectorXd QuadricFeature::eigenvalue_feature() const {
        ...
        ev_feature << (e1 - e2) / e1, (e2 - e3) / e1, e3 / e1, pow(e1 * e2 * e3, 1.0 / 3.0), (e1 - e3) / e1,
                -e1 * log(e1) - e2 * log(e2) - e3 * log(e3), e1 + e2 + e3, e3 / (e1 + e2 + e3);
        return ev_feature;
    }
    ...
    SurfaceFeature::Ptr SurfaceFeature::Random() {
        pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>);
        Eigen::Vector3d p1 = robot_utils::randomTranslation(1.0), p2 = robot_utils::randomTranslation(
                1.0), p3 = robot_utils::randomTranslation(1.0);
        for (int i = 0; i < 100; ++i) {
